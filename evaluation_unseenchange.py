...
from LAMDA_SSL.Dataset.Vision.imagenet import ImageNet32
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seen_ratio', type=float, default=0.2)
    ...
    if args.dataset == 'cifar-100':
        dataset = CIFAR100(root=args.root,
                          labeled_size=None, stratified=False, shuffle=False, download=True, default_transforms=True)
    if args.dataset == 'imagenet':
        dataset = ImageNet32(root=args.root,
    ...
    for _ in range(1):
        ...
        labeled_dataloader = LabeledDataLoader(batch_size=batch_size, num_workers=0, drop_last=True, worker_init_fn=worker_init, prefetch_factor=None)
        unlabeled_dataloader = UnlabeledDataLoader(num_workers=0, drop_last=True, worker_init_fn=worker_init, prefetch_factor=None)
        valid_dataloader = UnlabeledDataLoader(batch_size=batch_size, num_workers=0, drop_last=False, worker_init_fn=worker_init, prefetch_factor=None)
        test_dataloader = UnlabeledDataLoader(batch_size=batch_size, num_workers=0, drop_last=False, worker_init_fn=worker_init, prefetch_factor=None)
        ...
        if args.algo == 'Fix_A_Step_inverse':
            ...
            model = Fix_A_Step_inverse(
                mu=1, weight_decay=5e-4, lambda_u2=args.lambda_u2, threshold=args.threshold, low=args.low,
                high=args.high,
                eval_it=None,
                epoch=1, num_it_epoch=iteration, num_it_total=iteration,
                device=device,
