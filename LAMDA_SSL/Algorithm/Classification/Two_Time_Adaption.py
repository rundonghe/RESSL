...
class Two_Time_Adaption(InductiveEstimator,DeepModelMixin,ClassifierMixin):
    ...
    def train_unlabeled(self,lb_X,lb_y,ulb_X,lb_idx=None,ulb_idx=None,*args,**kwargs):
        ...
        weight_labeled = torch.Tensor(KMM(kernel_type='linear').fit(to_numpy(u_lb_features), to_numpy(u_w_ulb_features))).to('cuda:0').detach()
    ...
    def train_labeled(self,lb_X,lb_y,ulb_X,lb_idx=None,ulb_idx=None,*args,**kwargs):
        ...
        weight_unlabeled = torch.Tensor(
            KMM(kernel_type='linear').fit(to_numpy(l_w_ulb_features), to_numpy(l_lb_features))).to('cuda:0').detach()
        return l_lb_logits,lb_y,l_s_ulb_logits,weight_unlabeled,l_lb_features
    ...
    def get_loss_labeled(self,train_result,*args,**kwargs):
        l_lb_logits,lb_y,l_s_ulb_logits,weight_unlabeled,l_lb_features = train_result
        sup_loss=Cross_Entropy(reduction='mean')(logits=l_lb_logits,targets=lb_y)
        max_probs, targets_u = torch.max(self.it_pseudo_label, dim=-1)
        mask = max_probs.ge(self.threshold).float()
        unsup_loss = (Cross_Entropy(reduction='none')(l_s_ulb_logits, targets_u)*weight_unlabeled.detach() * mask).mean()
        # print(unsup_loss)
        loss=Semi_Supervised_Loss(lambda_u =self.lambda_u)(sup_loss,unsup_loss)
        return loss
