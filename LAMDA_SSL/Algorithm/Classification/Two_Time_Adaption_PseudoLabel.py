...
import cvxpy as cp
# from LAMDA_SSL.Network.TransferNet import MMDLoss
...
class Two_Time_Adaption(InductiveEstimator,DeepModelMixin,ClassifierMixin):
    ...
    def train_unlabeled(self,lb_X,lb_y,ulb_X,lb_idx=None,ulb_idx=None,*args,**kwargs):
        ...
        u_w_ulb_logits = u_logits[labeled_batch_size:]
        u_lb_features = u_features[:labeled_batch_size]
        u_w_ulb_features = u_features[labeled_batch_size:]
        ...
        weight_labeled = torch.Tensor(KMM(kernel_type='linear').fit(to_numpy(u_lb_features), to_numpy(u_w_ulb_features))).to(self.device).detach()
        return u_lb_logits,lb_y,u_w_ulb_logits,weight_labeled,u_lb_features
    ...
    def train_labeled(self,lb_X,lb_y,ulb_X,lb_idx=None,ulb_idx=None,*args,**kwargs):
        ...
        l_features, l_logits = self.labeled_network(inputs)
        l_lb_logits = l_logits[:labeled_batch_size]
        l_w_ulb_logits = l_logits[labeled_batch_size:]
        l_lb_features = l_features[:labeled_batch_size]
        l_w_ulb_features= l_features[labeled_batch_size:]
        # self.labeled_mean=(self.labeled_mean*self.labeled_num+torch.sum(l_lb_features,dim=0))/(self.labeled_num+labeled_batch_size)
        # self.labeled_num+=labeled_batch_size
        ...
        weight_unlabeled = torch.Tensor(
            KMM(kernel_type='linear').fit(to_numpy(l_w_ulb_features), to_numpy(l_lb_features))).to(self.device).detach()
        return l_lb_logits,lb_y,l_w_ulb_logits,weight_unlabeled,l_lb_features

    def get_loss_unlabeled(self,train_result,*args,**kwargs):
        u_lb_logits,lb_y,u_w_ulb_logits,weight_labeled,u_lb_features = train_result
        sup_loss=(Cross_Entropy(reduction='none')(logits=u_lb_logits,targets=lb_y)*weight_labeled.detach()).mean()
        # print(sup_loss)
        _warmup = float(np.clip((self.it_total) / (self.warmup * self.num_it_total), 0., 1.))
        self.it_pseudo_label = torch.softmax(u_w_ulb_logits, dim=-1)
        max_probs, max_idx = torch.max(self.it_pseudo_label, dim=-1)
        mask = max_probs.ge(self.threshold).float()
        unsup_loss = _warmup*(Cross_Entropy(reduction='none')(u_w_ulb_logits, max_idx.detach())*mask).mean() # MSE loss for unlabeled data
        # print(unsup_loss)
        loss=Semi_Supervised_Loss(lambda_u =self.lambda_u)(sup_loss,unsup_loss)
        return loss

    def get_loss_labeled(self,train_result,*args,**kwargs):
        l_lb_logits,lb_y,l_w_ulb_logits,weight_unlabeled,l_lb_features = train_result
        sup_loss=Cross_Entropy(reduction='mean')(logits=l_lb_logits,targets=lb_y)
        max_probs, max_idx = torch.max(self.it_pseudo_label, dim=-1)
        mask = max_probs.ge(self.threshold).float()
        unsup_loss = (Cross_Entropy(reduction='none')(l_w_ulb_logits, max_idx.detach())*weight_unlabeled.detach() * mask).mean()
        # print(unsup_loss)
        loss=Semi_Supervised_Loss(lambda_u =self.lambda_u)(sup_loss,unsup_loss)
        return loss
    ...
    def estimate(self, X, idx=None, *args, **kwargs):
        _, outputs = self.unlabeled_network(X)
        return outputs
