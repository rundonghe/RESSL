...
from LAMDA_SSL.Algorithm.Classification.AdversarialNet import AdversarialNet
import copy
from LAMDA_SSL.Scheduler.InverseDecaySheduler import InverseDecaySheduler
from LAMDA_SSL.Opitimizer.SGD import SGD
import numpy as np
from LAMDA_SSL.Algorithm.Classification.Default_Config import config
import torch
...
def inverseDecaySheduler(step, initial_lr, gamma=10, power=0.75, max_iter=2000):
    return initial_lr * ((1 + gamma * min(1.0, step / float(max_iter))) ** (- power))

def compute_score(inputs, model, eps,mean):
    model.eval()
...
discriminator_optimizer=SGD(lr=5e-4, momentum=0.9)
discriminator_optimizer_separate=SGD(lr=5e-4, momentum=0.9)
discriminator_scheduler=InverseDecaySheduler(num_training_steps=2000)
discriminator_scheduler_separate=InverseDecaySheduler(num_training_steps=2000)
...
class CAFA(DeepModelMixin,InductiveEstimator,ClassifierMixin):
    def __init__(self,lambda_u=1.0,
                 ...
                 discriminator_separate=discriminator_separate,
                 discriminator_optimizer=discriminator_optimizer,
                 discriminator_optimizer_separate=discriminator_optimizer_separate,
                 discriminator_scheduler=discriminator_scheduler,
                 discriminator_scheduler_separate=discriminator_scheduler_separate,
                 epoch=1,
                 num_it_epoch=1000,
                 num_it_total=1000,
                 eval_epoch=None,
    ...
    def train(self,lb_X,lb_y,ulb_X,lb_idx=None,ulb_idx=None,*args,**kwargs):
        ...
        self.label_all[lb_idx]=lb_y
        l_feature,lb_logits = self._network(lb_X)
        self.bn_controller.freeze_bn(self._network)
        u_feature,w_ulb_logits = self._network(ulb_X)
        self.bn_controller.unfreeze_bn(self._network)

        self._network.eval()
        l_pred_shift = compute_score(lb_X.detach(), self._network,self.eps,mean=self.mean).detach()
        u_pred_shift = compute_score(ulb_X.detach(),self._network,self.eps,mean=self.mean).detach()
        self._network.train()
        ...
        #print(len(self._train_dataset.labeled_dataset))
        if self.it_total%((len(self._train_dataset.labeled_dataset)-1)//lb_X.shape[0]+1)==0:
        ...
        _, s_ulb_logits = self._network(ulb_X)
        if self.warmup is not None:
            _warmup = float(np.clip((self.it_total) / (self.warmup * self.num_it_total), 0., 1.))
            self.lambda_u=self.lambda_u * _warmup
        # adv_grads=torch.autograd.grad(adv_loss,(self._network.parameters()),create_graph=True,allow_unused=True)
        ssl_loss =  self.lambda_u* (F.mse_loss(w_ulb_logits.softmax(1),s_ulb_logits.softmax(1).detach(),reduction='none').mean(1)*unlabel_share_weight).mean()
        # supervised loss
        cls_loss = Cross_Entropy(reduction='mean')(logits=lb_logits,targets=lb_y)
        # grads = torch.autograd.grad(cls_loss, (self._network.parameters()), create_graph=True, allow_unused=True)
        # print(grads)
        return cls_loss , ssl_loss ,  adv_loss , adv_loss_separate
    ...
    def get_loss(self,train_result,*args,**kwargs):
        ...
        cls_loss, ssl_loss,  adv_loss, adv_loss_separate=train_result
        if self.warmup is not None:
            coef = 1. * math.exp(-5 * (1 - min(self.it_total / (self.warmup * self.num_it_total), 1)) ** 2)
        else:
            coef=1
        if self.adv_warmup is not None:
            adv_coef = 1. * math.exp(-5 * (1 - min(self.it_total / (self.warmup * self.num_it_total), 1)) ** 2)
        else:
            adv_coef=1
        loss = cls_loss + coef*ssl_loss + adv_coef * (adv_loss + adv_loss_separate)
        return loss
