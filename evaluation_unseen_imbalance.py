...
def imbalance_sample(T_X, T_y, imb_factor, unseen_ratio, seed, unseen_class_num):
    np.random.seed(seed)
    random.seed(seed)
    unlabels_num = T_X.shape[0]
    unlabels_unseen_num = unlabels_num * unseen_ratio
    cls_num = unseen_class_num

    img_max = unlabels_unseen_num / cls_num
    img_num_per_cls = []
    ...
    new_data = []
    new_targets = []
    targets_np = np.array(T_y)
    classes = np.unique(targets_np)
    np.random.shuffle(classes)
    for the_class, the_img_num in zip(classes, img_num_per_cls):
        idx = np.where(T_y == the_class)[0]
        np.random.shuffle(idx)
        selec_idx = idx[:the_img_num]
        new_data.append(T_X[selec_idx, ...])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unseen_ratio', type=float, default=0.4)
    ...
    for seed in range(3):
        ...
        ib_T_X, ib_T_y = imbalance_sample(T_X, T_y, imb_factor=args.imb_factor, unseen_ratio=unseen_ratio, seed=seed, unseen_class_num=args.unseen_class_num)
        ...
        target_X_out, target_y_out = ib_T_X, ib_T_y
        if unseen_ratio == 0:
